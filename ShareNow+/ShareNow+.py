...
def get_clipboard_content():
    ...
    # This comes before generic text/plain from wl-paste to prefer rich text
    if os.getenv('WAYLAND_DISPLAY') or os.getenv('XDG_SESSION_TYPE') == 'wayland':
        try:
            ...
            # If wl-paste is not found at all, we should note it
        ...
        try:
            ...
            if markdown_content_raw:
                ...
                # If you install 'markdown' library (pip install markdown), you can convert it to HTML here:
                # try:
                #     detected_content['text_html'] = markdown.markdown(detected_content['text_markdown'])
                #     app.logger.info("Clipboard content: Markdown converted to HTML.")
                # except Exception as md_conv_e:
                #     app.logger.warning(f"Failed to convert Markdown to HTML: {md_conv_e}")
                #     detected_content['text_markdown_error'] = f"Markdown conversion to HTML failed: {md_conv_e}"
        except (subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired) as e:
            app.logger.debug(f"wl-paste --type text/markdown failed: {e}")
            # If wl-paste is not found, it will be caught by text/html_error or image error later
        except Exception as e:
    ...
    # List of MIME types to try, in order of preference. SVG handles separately if found.
    IMAGE_MIME_TYPES = ['image/png', 'image/jpeg', 'image/bmp', 'image/gif', 'image/tiff', 'image/webp']
    image_data_found = False
    image_access_errors = [] # Collect errors for image access
    ...
    if os.getenv('WAYLAND_DISPLAY') or os.getenv('XDG_SESSION_TYPE') == 'wayland':
        ...
        # Then, try other raster image formats and convert to PNG
        if not detected_content.get('image_png_base64'): # Only try other images if PNG not already found
                ...
                try:
                    ...
                    if raw_image_data:
                        ...
                        try:
                            ...
                            image_data_found = True
                            break # Found and processed an image, no need to try other types
                    ...
                    if isinstance(e, FileNotFoundError):
                        ...
                        break # If wl-paste itself is not found, no point trying other types
        ...
        # Set final image error message if no images were successfully found/converted
        if not image_data_found and not detected_content.get('image_svg_base664') and image_access_errors: # Corrected from image_svg_base664
             detected_content['image_error'] = "No convertible image found or errors occurred: " + "; ".join(set(image_access_errors)) 
        elif not image_data_found and not detected_content.get('image_svg_base64'): # Corrected from image_svg_base664
...
def index():
    ...
    return render_template_string(TEMPLATE, clipboard_data=clipboard_data, SHARE_CLIPBOARD=SHARE_CLIPBOARD)
...
<div id="container">
    <h2>ShareNow+ v1.10 (Text, HTML, Image Clipboard)</h2>
    <div id="pathbar">
    ...
    <div class="section" id="clipboard-section">
        <div id="clipboard-header">
            ...
            <button type="button" class="btn clipboard-action-btn" id="refreshClipboardBtn">Refresh Clipboard</button>
        </div>
        <div id="clipboard-content">
            {# Initial clipboard content rendered by Flask #}
            {% if SHARE_CLIPBOARD %} {# Only try to display if clipboard sharing is enabled #}
                ...
                {% if clipboard_data.text_html %}
                    <div class="clipboard-content-item">
                        ...
                        <div style="border: 1px dashed #ccc; padding: 5px; max-height: 200px; overflow-y: auto;">
                            {{ clipboard_data.text_html | safe }} {# Render HTML directly. BE CAREFUL with untrusted HTML! #}
                        </div>
                        <p style="font-size: 0.8em; color: #666;">(Rendered directly. Download for source.)</p>
                        <button type="button" class="btn clipboard-action-btn" onclick="copyToClientClipboard('{{ clipboard_data.text_html | js_string }}')">Copy HTML to My Clipboard</button> {# Added HTML copy button #}
...
const uploadDirBtn    = document.getElementById('dirUploadBtn');
    ...
    .then(resp => {
        if (!resp.ok) { 
            return resp.json().then(errorData => {
                ...
                // Attempt to revert to previous valid path or root on error
...
function updateClipboardContent(clipboardData) {
    ...
    if (clipboardData.text_html) {
        hasContent = true;
        // Escape HTML for the JS string literal in onclick, but not for innerHTML itself
        const escapedHtmlForJs = JSON.stringify(clipboardData.text_html).slice(1, -1);
        contentHtml += `
            <div class="clipboard-content-item">
                <h5>HTML Content:</h5>
                <div style="border: 1px dashed #ccc; padding: 5px; max-height: 200px; overflow-y: auto;">
                    ${clipboardData.text_html} {# HTML is inserted directly. CONSIDER SANITIZATION FOR UNTRUSTED INPUT! #}
                </div>
                <p style="font-size: 0.8em; color: #666;">(Rendered directly. Download for source.)</p>
...
if (refreshClipboardBtn) {
    refreshClipboardBtn.onclick = () => {
        clipboardContentDiv.innerHTML = '<p>Refreshing clipboard content...</p>'; // Show loading message
        fetch('/api/clipboard/refresh')
            .then(response => {
                if (!response.ok) {
                    return response.json().then(errorData => {
                        const errorMessage = errorData.error || response.statusText;
                        clipboardContentDiv.innerHTML = `<p style="color:red;">Failed to refresh clipboard: ${escapeHtml(errorMessage)}</p>`;
            ...
            .catch(error => {
                ...
                // Error message already set by the .then(response => ...) block
            });
    };
}
