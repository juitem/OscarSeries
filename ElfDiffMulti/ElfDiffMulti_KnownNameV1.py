...
def classify_section(mode, stype, sflag, sname,
                     berkeley_known_name=False,
                     gnu_known_name=False):
    """
    Classify ELF section into 'text', 'data', 'bss', or 'others'.

    If berkeley_known_name is True and mode == 'berkeley', use expanded
    known-name mapping to match traditional `size -A` Berkeley output.
    If gnu_known_name is True and mode == 'gnu', use similar mapping for GNU.
    Otherwise, fallback to original flag/type-based classification.
    """

    A = bool(sflag & 0x2)  # ALLOC
    W = bool(sflag & 0x1)  # WRITE
    X = bool(sflag & 0x4)  # EXECINSTR
    PROGBITS = (stype == 'SHT_PROGBITS')
    NOBITS = (stype == 'SHT_NOBITS')

    # -----------------------------
    # Berkeley known-name override
    # -----------------------------
    if mode == "berkeley" and berkeley_known_name:
        text_secs = {
            ".interp", ".init", ".plt", ".plt.got", ".plt.sec", ".text", ".fini",
            ".rodata", ".rodata1",
        ...
        data_secs = {
            ".dynamic", ".got", ".got.plt", ".data", ".data1", ".data.rel.ro",
            ".ctors", ".dtors", ".jcr"
        }
        bss_secs = {".bss", ".tbss", ".sbss", ".sbss2"}

        if sname in text_secs:
            return "text", stype
        elif sname in data_secs:
            return "data", stype
        elif sname in bss_secs:
            return "bss", stype
        else:
            return "others", stype

    # -----------------------------
    # GNU known-name override
    # -----------------------------
    if mode == "gnu" and gnu_known_name:
        text_secs = {
            ".interp", ".init", ".plt", ".plt.got", ".plt.sec", ".text", ".fini",
            ".rodata", ".rodata1",
            ".eh_frame_hdr", ".eh_frame", ".gcc_except_table",
            ".hash", ".gnu.hash", ".dynsym", ".dynstr",
            ".gnu.version", ".gnu.version_r", ".note.gnu.build-id",
            ".note.ABI-tag"
        }
        data_secs = {
            ".dynamic", ".got", ".got.plt", ".data", ".data1", ".data.rel.ro",
            ".ctors", ".dtors", ".jcr", ".tdata", ".tbss"
        }
        bss_secs = {".bss", ".tbss", ".sbss", ".sbss2"}
...
#                        print(f"[DEBUG] File: {rel}, Section: {sec_name}, Class: {cls}, Size: {sz}")
...
def aggregate_classification_by_class(df):
    ...
    order = ["text", "data", "bss", "others", "User-Selected"]
    grouped = df.groupby('class', sort=False).agg({
        'section name': lambda x: ', '.join(sorted(set(x))),
        'type': lambda x: ', '.join(sorted(set(str(i) for i in x))),
        'flags': lambda x: ', '.join(sorted(set(str(i) for i in x)))
    }).reset_index()

    # Re-apply categorical order after grouping, then sort
    grouped['class'] = pd.Categorical(grouped['class'], categories=order, ordered=True)
    grouped = grouped.sort_values('class').reset_index(drop=True)
    return grouped
...
def add_top_n_diff_boxes(ax, df, top_n, fontsize=9):
    """
    Add four semi-transparent top-N diff boxes at chart corners:
    +diff (top-left), -diff (top-right), +diff% (bottom-left), -diff% (bottom-right).
    """
    boxes_data = []
...
def human_readable_log_formatter(x, pos):
    if x <= 0:
        return ""
    return human_readable_size(int(x))
...
#     except:
#         return str(x)
#     return human_readable_size(x_int)
...
def make_section_diff_chart_with_log_y(
    size1_list, size2_list, diff_list, diff_pct_list, labels, title, path,
    top_n=10, use_log_scale=True
):
    print("[DEBUG] Chart input ")
    for i in range(len(labels)):
        print(f"Section: {labels[i]}, size1: {size1_list[i]}, size2: {size2_list[i]}, diff: {diff_list[i]}, diff%: {diff_pct_list[i]:.2f}")
    ...
    if use_log_scale:
        # Use absolute values for log scale, but avoid zero
        bar_values = [abs(d) if d > 0 else 1 for d in diff_list]
        ax.set_yscale("log")
        ax.yaxis.set_major_formatter(FuncFormatter(human_readable_log_formatter))
        ax.text(
            0.55, 0.95, "Log Scale Y-axis", transform=ax.transAxes,
            fontsize=10, color='red', ha='right', va='bottom',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.7, edgecolor='red')
        )
    else:
        bar_values = [abs(d) for d in diff_list]
    ...
    #####
    desired_order = ['text', 'data', 'bss', 'others', 'User-Selected']

    # DataFrame의 행 순서를 원하는 대로 변경합니다.
    combined_df['section'] = pd.Categorical(combined_df['section'], categories=desired_order, ordered=True)
    combined_df = combined_df.sort_values('section').reset_index(drop=True)
     #####

    add_top_n_diff_boxes(ax, combined_df, top_n)
...
def save_md_html_reports(df_dict, top_files_dict, class_tables, output_dir, prefix, cmd, include_topfiles=False):
    ...
    for mode in df_dict:
        ...
        # Classification Table with sorted order
        if class_tables.get(mode) is not None and not class_tables[mode].empty:
            agg_df = class_tables[mode].groupby('class').agg({
                'section name': lambda x: ', '.join(sorted(set(x))),
                'type': lambda x: ', '.join(sorted(set(str(i) for i in x))),
            ...
            # Sorting order: text -> data -> bss -> others -> User-Selected
            order = ["text", "data", "bss", "others", "User-Selected"]
            agg_df['class'] = pd.Categorical(agg_df['class'], categories=order, ordered=True)
            agg_df = agg_df.sort_values('class').reset_index(drop=True)

            md_content += "### Classification Table\n"
            md_content += md_format_table(agg_df, ['class', 'section name', 'type', 'flags']) + "\n"

        # Include Top Files if requested
        if include_topfiles:
            md_content += f"### Top Files ({mode})\n"
            for sec, files in top_files_dict.get(mode, {}).items():
                md_content += f"#### {sec}\n"
                ...
                if rows:
                    md_content += md_format_table(pd.DataFrame(rows),
    ...
    # Save HTML file (if markdown package available)
    try:
        import markdown
        html_content = markdown.markdown(md_content, extensions=['tables', 'fenced_code'])
        html_filepath = os.path.join(output_dir, f"{prefix}{'_topfiles' if include_topfiles else ''}.html")
...
def generate_report(class_tables, output_dir):
    """
    Generate Markdown and HTML reports from classification tables.

    Args:
    - class_tables: dict of {mode: DataFrame}, where each DataFrame contains classification details
    - output_dir: directory path to save the generated reports
    """
    for mode, df in class_tables.items():
        # Aggregate by class for the report summary
        ...
        # Specify sorting order (text, data, bss, others, User-Selected)
        order = ["text", "data", "bss", "others", "User-Selected"]
        agg_df['class'] = pd.Categorical(agg_df['class'], categories=order, ordered=True)
        agg_df = agg_df.sort_values('class').reset_index(drop=True)

        # Generate Markdown report
        md_report_path = f"{output_dir}/{mode}_classification_report.md"
        with open(md_report_path, "w", encoding="utf-8") as md_f:
            md_f.write(f"# {mode.capitalize()} Classification Report\n\n")
            md_f.write(agg_df.to_markdown(index=False))

        # Generate HTML report
        html_report_path = f"{output_dir}/{mode}_classification_report.html"
        with open(html_report_path, "w", encoding="utf-8") as html_f:
            html_f.write(f"<h1>{mode.capitalize()} Classification Report</h1>\n")
            html_f.write(agg_df.to_html(index=False))

        print(f"[INFO] Report generated for {mode}:")
...
def main():
    ...
    ap.add_argument("--berkeley-known-name", action="store_true", default=False,
                    help="Use known section-name mapping for Berkeley mode classification")
    ap.add_argument("--gnu-known-name", action="store_true",default=False,
                    help="Use known section-name mapping for GNU mode classification")
    berkeley_known_name = False
    gnu_known_name = False
    ...
    berkeley_known_name = args.berkeley_known_name
    gnu_known_name = args.gnu_known_name
